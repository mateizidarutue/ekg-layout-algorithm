...
Datasets are declared in `output/manifest.json`. This file is **not committed** — the `output/` directory is empty in a fresh checkout (only `.gitkeep`). Create the manifest the first time you export a bundle:
...
If `manifest.json` is absent, the viewer still loads — the "Switch dataset" modal falls back to an "Open local JSON file…" affordance you can use to load a bundle directly from disk.
...
|       `-- bpic19_DS.json
|-- output/
|   `-- manifest.json          <- dataset registry (NOT in git; create on first export)
|-- pipeline/
|   |-- build_ekg.py
|   |-- export_json.py
|   |-- config_loader.py
...
All commands below assume you are at the repository root (the `bep/` directory inside this checkout). On the development machine that is:
 
```powershell
cd C:\Users\matei\Documents\GitHub\complex_bep\bep
```
 
On any other machine, substitute the actual checkout path.
...
1. load `config.yaml` and resolve the dataset's semantic header and dataset-description paths,
2. create a temporary working directory and mirror the expected data paths for PromG (via directory junctions on Windows, symlinks on Unix),
3. connect to the dataset-specific Neo4j database (`datasets.<name>.database` in `config.yaml`),
4. clear the database and set constraints,
5. import raw records,
6. create nodes (events + entities),
7. create relations (CORR + structural entity-to-entity),
8. infer DF edges where the semantic header requests them.
Important command:
...
- `output/manifest.json` is not committed to git; create it the first time you export a bundle, or use the viewer's "Open local JSON file…" fallback
